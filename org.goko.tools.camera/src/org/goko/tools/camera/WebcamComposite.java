...
public class WebcamComposite extends Composite implements WebcamListener, PaintListener {
	...
	public WebcamComposite(Composite parent, int style) {
		...
		setSize(640, 480);
		setVisible(true);
		setBackground(new Color(Display.getDefault(), new RGB(0, 0, 0)));
		...
		if(image != null){
			...
			if(fillArea){
				...
				if(getSize().x < getSize().y){
					...
					y = (getSize().y / 2) - (height / 2);
					
				}else if(getSize().x > getSize().y){				
					height = getSize().y; 
					width =  (int) (height * ratio);
					x = (getSize().x / 2) - (width / 2);
				}
	 ...
	 * @param x 
	 */
	private void drawCrosshair(GC gc, int x, int y, int width, int height) {
