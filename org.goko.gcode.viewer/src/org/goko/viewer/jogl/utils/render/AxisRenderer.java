...
public class AxisRenderer implements IJoglRenderer {
	...
	public AxisRenderer() {
		GLUT glut = new GLUT();
		textScale = CHAR_WIDTH
				/ glut.glutStrokeWidth(GLUT.STROKE_MONO_ROMAN, ' ');
	...
	public void render(JoglRendererProxy proxy) throws GkException {
		...
		proxy.getGl().glTranslated(xaxis.getX().doubleValue()-1, xaxis.getY().doubleValue(), xaxis.getZ().doubleValue());
		proxy.getGl().glPushMatrix();
		...
		proxy.getGl().glTranslated(yaxis.getX().doubleValue(), yaxis.getY().doubleValue() - 1, yaxis.getZ().doubleValue());
		proxy.getGl().glPushMatrix();
		...
		proxy.getGl().glTranslated(zaxis.getX().doubleValue(), zaxis.getY().doubleValue(), zaxis.getZ().doubleValue() - 1);
		proxy.getGl().glPushMatrix();
