 ...
 * @date 7 juin 2016
 */
public class QueueReportVerbosityFix implements ITinyGConfigurationFix<G2CoreConfiguration>{
	/** LOG */
	...
	public boolean shouldApply(G2CoreConfiguration configuration) {
		try {			
			BigDecimal setting = configuration.getSetting(G2Core.Configuration.Groups.SYSTEM, G2Core.Configuration.System.QUEUE_REPORT_VERBOSITY, BigDecimal.class);
			return !ObjectUtils.equals(G2Core.Configuration.Values.QUEUE_REPORT_VERBOSE, setting);
		} catch (GkException e) {
			LOG.error(e);
		}
		return false;
	...
	public void apply(G2CoreConfiguration configuration) {
		try{			
			configuration.setSetting(G2Core.Configuration.Groups.SYSTEM, G2Core.Configuration.System.QUEUE_REPORT_VERBOSITY, G2Core.Configuration.Values.QUEUE_REPORT_VERBOSE);			
		} catch (GkException e) {
			LOG.error(e);
		}
	}
	...
	public String getDescription() {		
		return "Queue Report Verbosity parameter ($qv) will be set to 2 - Verbose";
	}
