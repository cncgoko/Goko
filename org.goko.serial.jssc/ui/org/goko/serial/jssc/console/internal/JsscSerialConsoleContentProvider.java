...
public class JsscSerialConsoleContentProvider implements StyledTextContent {
	 ...
	 */
	JsscSerialConsoleContentProvider() {
		super();
		setText("");
	}
	 ...
	 *         valid
	 */
	protected boolean isValidReplace(int start, int replaceLength,
			String newText) {
		if (replaceLength == 0) {
			...
			if (endChar == '\r') {
				...
				if (start + replaceLength != getCharCount()) {
					char after = getTextRange(start + replaceLength, 1).charAt(
							0);
					if (after == '\n') {
						return false;
					}
	...
	void moveAndResizeGap(int position, int size, int newGapLine) {
		...
		if (newSize < 0) {
			if (oldSize > 0) {
				...
				System.arraycopy(textStore, gapEnd, content, gapStart,
	...
	public String getLine(int index) {
		...
		if (!gapExists() || (end < gapStart) || (start >= gapEnd)) {
			...
			while ((length - 1 >= 0)
					&& isDelimiter(textStore[start + length - 1])) {
				length--;
			}
			...
			buf.append(textStore, gapEnd, length - gapLength
					- (gapStart - start));
			length = buf.length();
	...
	String getFullLine(int index) {
		...
		if (!gapExists() || (end < gapStart) || (start >= gapEnd)) {
			...
			buffer.append(textStore, gapEnd, length - gapLength
					- (gapStart - start));
	...
	public void removeTextChangeListener(TextChangeListener listener) {
		...
		for (int i = 0; i < textListeners.size(); i++) {
			TypedListener typedListener = (TypedListener) textListeners
					.elementAt(i);
	...
	void sendTextEvent(StyledTextEvent event) {
		for (int i = 0; i < textListeners.size(); i++) {
			((StyledTextListener) textListeners.elementAt(i))
					.handleEvent(event);
		}
	}
	...
	void delete(int position, int length, int numLines) {
		...
		int[][] oldLines = indexLines(position, length + (gapEnd - gapStart),
				numLines);
